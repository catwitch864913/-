...
public class 仙草箭矢 : MonoBehaviour
{
    public int damge;
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Enemy"))
        {
            ...
            {
                collision.GetComponent<魔族小兵>().TakeDamage(20);
            }
            if (mosterScript != null)
            {
                collision.GetComponent<怪物>().TakeDamage(damge);
            }
            if (FairyLeader != null)
            {
        ...
        {
            collision.GetComponent<怪物2>().TakeDamage(20);
            Destroy(gameObject);
            音效.箭命中人();
        }
        if (collision.gameObject.tag == "Boss")
        {
            collision.GetComponent<BOSS>().TakeDamage(20);
            Destroy(gameObject);
            音效.箭命中人();
        }
